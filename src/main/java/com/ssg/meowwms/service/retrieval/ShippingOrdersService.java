...
public interface ShippingOrdersService {
     ...
     * */
    void registerShippingOrder(ShippingOrdersDTO shippingOrdersDTO, ShippingOrdersDetailDTO shippingOrdersDetailDTO);
     ...
     * */
    ShippingOrdersDTO getShippingOrder(int id);

    /**
     * 출고 지시서 수정 : 출고 지시서 상세 페이지에서 지시서 내용을 수정할 수 있는 메서드입니다.
     * */
    void modifyShippingOrder(ShippingOrdersDTO shippingOrdersDTO);
     ...
     */
    List<VehicleDTO> manageDispatch(List<OptionDTO> options);

    /**
     * 배차 확정 : 배차 관리 창에서 확인 버튼을 누르면 배차가 확정됩니다.
     */
    void confirmDispatch(ShippingOrdersStatusDTO shippingOrdersStatusDTO, VehicleDTO vehicleDTO);

    /**
     * 출고 승인 (전 확인): 페이지에서 출고 승인 버튼을 누르면 모달 창을 띄우는 서비스입니다.
     * 배차 관리가 확정됐는지 필터링 하며 승인 최종 확정의 전 단계입니다.
     * */
    List<Integer> approveRetrieval(int id);
